"""
TESTNET DENEME ARACI
====================
Actions -> Testnet Test ile elle calistirilir.
  durum  : sadece rapor (bakiye, acik pozisyon, bekleyen emirler)
  market : HEMEN pozisyon acar + stop/tp yerlestirir
  limit  : fiyatin %3 uzagina limit emir koyar (dolmayi bekler)
"""
...
for e in ae:
    print(f"   {e.get('type'):22s} {e.get('side'):5s} @ "
          f"{e.get('stopPrice') or e.get('price')}")
if mod != "durum" and not any(e.get("type") == "STOP_MARKET" for e in ae):
    p2 = T.pozisyon_var_mi(sym)
    if p2:
        print("   [!] UYARI: acik pozisyon var ama STOP emri YOK")
print("=" * 60)
