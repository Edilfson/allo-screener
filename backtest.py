...
def run_config(data, p, btc_lookup):
    ...
    if not trades:
              return {"trades": 0, "total_r": 0.0, "avg_r": 0.0, "win_rate": 0.0,
                                      "max_dd": 0.0, "bull_r": 0.0, "bull_n": 0, "bear_r": 0.0, "bear_n": 0}
    ...
    return {
              ...
              "bull_r": float(sum(bull)), "bull_n": len(bull),
              "bear_r": float(sum(bear)), "bear_n": len(bear),
    }
...
def cfg_label(r):
      return (f"{r['ma']} tol%{r['touch_tol']*100:.1f} RR{r['min_rr']:.0f} "
                          f"{r['entry_mode']} {r['stop_mode']} {'kalite+' if r['quality'] else 'kalite-'}")
...
def main():
    ...
    L = [f"🧪 <b>BACKTEST v2</b> | ~{period_days:.0f} gun | {BT_INTERVAL} | "
                  f"{len(data)} coin | {len(combos)} komb.\n", "<b>En iyi 8:</b>"]
    for r in pool[:8]:
              L.append(f"{cfg_label(r)}\n  → {r['total_r']:+.1f}R | {r['trades']}isl | "
                                        f"ort {r['avg_r']:+.2f}R | win%{r['win_rate']:.0f} | dd{r['max_dd']:.1f}\n"
                                        f"  🐂 {r['bull_r']:+.1f}R ({r['bull_n']}isl)  🐻 {r['bear_r']:+.1f}R ({r['bear_n']}isl)")
    ...
    tb_r = sum(r["bull_r"] for r in results); tb_n = sum(r["bull_n"] for r in results)
    ta_r = sum(r["bear_r"] for r in results); ta_n = sum(r["bear_n"] for r in results)
    if tb_n or ta_n:
              L.append(f"\n<b>Tum islemler rejim kirilimi:</b>\n"
                                        f"  🐂 boga: {tb_n} islem, islem basi {tb_r/max(tb_n,1):+.2f}R\n"
                                        f"  🐻 ayi: {ta_n} islem, islem basi {ta_r/max(ta_n,1):+.2f}R")

    L.append("\n<i>Uyari: tek doneme asiri uyum riski - kumelerin ortak yonune bak, "
                          "iki zaman diliminde de tutarli olani sec.</i>")
